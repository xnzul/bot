...
const { color, bgcolor } = require('./R00TXNZUL/color')
const { help } = require('./R00TXNZUL/help')
const kagApi = require('@kagchi/kag-api')
const { donasi } = require('./R00TXNZUL/donasi')
const { fetchJson } = require('./R00TXNZUL/fetcher')
const { recognize } = require('./R00TXNZUL/ocr')
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./R00TXNZUL/functions')
const tiktod = require('tiktok-scraper')
const ffmpeg = require('fluent-ffmpeg')
const { removeBackgroundFromImageFile } = require('remove.bg')
const welkom = JSON.parse(fs.readFileSync('./R00TXNZUL/welkom.json'))
const nsfw = JSON.parse(fs.readFileSync('./R00TXNZUL/nsfw.json'))
const samih = JSON.parse(fs.readFileSync('./R00TXNZUL/simi.json'))
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            + 'VERSION:3.0\n' // NGAPAIN LAGI KALO GA MAU NUMPANG NAMA DOANG XIXIXIXI
            + 'FN:CO XNZULBOT\n' // MENDING LU TOBAT SU!
            + 'ORG:Creator XNZULBOT;\n' // KASIH CREDITS GUA SU!!!
...
const config = {
    NZUL: '👾XNZULBOT👾', // TOBAT SU ASU
    instagram: 'https://instagram.com/r00t_xnzul', // INFO JANGAN DI UBAH
    nomer: 'https://wa.me/6283173142772?text=XNzul+lofyu+><+', // INFO SU JNGAN DI UBAH
    youtube: 'https://youtube.com/c/XNzulMods', // KINTIL
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❕❭ 𝐖𝐀𝐈𝐓 𝐏𝐑𝐎𝐒𝐄𝐒',
				success: '️❬ ✅ ❭ 𝐒𝐔𝐂𝐂𝐄𝐒𝐒, 𝐉𝐀𝐍 𝐋𝐔𝐏𝐀 𝐃𝐎𝐍𝐀𝐒𝐈 𝐊𝐀𝐊🖤',
				error: {
			...
			const ownerNumber = ["623173142772@s.whatsapp.net"] // ganti nomer lu
           ...
           case '#demote':
              ...
              case '#abangjago':
              case '#abgjago':
                    let eerrr = fs.readFileSync('./R00TXNZUL/abangjago.mp3')
				...
				case '#tts':
					...
					const gtts = require('./R00TXNZUL/gtts')(args[0])
				...
				case '#simi':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝐒𝐔𝐂𝐂𝐒𝐄𝐒𝐒 ❭ 𝐦𝐞𝐧𝐠𝐚𝐤𝐭𝐢𝐟𝐤𝐚𝐧 𝐟𝐢𝐭𝐮𝐫 𝐬𝐢𝐦𝐢 𝐩𝐚𝐝𝐚 𝐠𝐫𝐨𝐮𝐩 𝐢𝐧𝐢️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('❬ 𝐒𝐔𝐂𝐂𝐒𝐄𝐒𝐒 ❭ 𝐦𝐞𝐧𝐨𝐧𝐚𝐤𝐭𝐢𝐟𝐤𝐚𝐧 𝐟𝐢𝐭𝐮𝐫 𝐬𝐢𝐦𝐢 𝐩𝐚𝐝𝐚 𝐠𝐫𝐨𝐮𝐩 𝐢𝐧𝐢️️')
					} else {
						reply('𝐤𝐞𝐭𝐢𝐤 𝟏 𝐮𝐧𝐭𝐮𝐤 𝐦𝐞𝐧𝐠𝐚𝐤𝐭𝐢𝐟𝐤𝐚𝐧, 𝟎 𝐮𝐧𝐭𝐮𝐤 𝐦𝐞𝐧𝐨𝐧𝐚𝐤𝐭𝐢𝐟𝐤𝐚𝐧 𝐟𝐢𝐭𝐮𝐫')
					}
					break 
